...
We derive a non-equilibrium efficiency bound for displacement processes against an entropic-gravity-style potential in BEC analog systems. Under one explicit thermodynamic assumption (the screen exchanges heat reversibly while the reservoir coupling carries the irreversibility), the Clausius inequality applied to a two-temperature process yields η ≤ 1 − T/T_res. The bound becomes empirically discriminating only when T/T_res ~ O(1), a regime accessible in cold-atom analog gravity but not in astrophysics. For laboratory BEC parameters the bound predicts a 20% suppression below naive energetic accounting, distinguishable from mundane experimental losses by its specific functional form. The load-bearing substitution is examined across five QuTiP Lindblad simulations; for thermal initial states it is an algebraic identity that the runs reproduce within numerical error (a consistency check on the integrator, not an independent test), and it fails for non-thermal initial states with macroscopic coherences — a clean scope limit. Simulation scripts not yet publicly archived; the numerical method is fully specified in the paper.
